...
void Sender::sendMessage()
{
	try
	{
		...
		//std::string s = msg.toString();
		//std::cout << "IN THE MAIN SEND FUNCTION\n";
		...
		if (message["commandType"] == "File_Upload")
		{
			if (connect(std::stoi(message["receiverPort"])))	//Connected to the peer.
			{
				if (sendFile(msg))
				{
					Verbose::show("File:"+msg.getMessage()["filePath"]+" Sent from Peer: "+msg.getMessage()["senderPort"]+" to Peer:"+msg.getMessage()["receiverPort"]);
				}
				else
				{
